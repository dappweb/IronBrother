...
const USER_DYNAMIC_SETTLEMENT_FALLBACK_BATCH_SIZE = 10;
...
function BotRewardsScreen({ address, data, locale }: { address?: Address; data: ReturnType<typeof useIronBrotherData>; locale: LocaleKey }) {
  ...
  const pendingSettlementBatches = useMemo(() => {
    ...
    return batches;
  }, [visiblePendingRows, userSettlementBatchSize]);
  ...
  function settlementBatchesForRows(rowsToSettle: readonly PendingDynamicRewardRow[]) {
    ...
    const batches: { source: Address; day: bigint }[][] = [];
    for (let index = 0; index < uniqueRows.length; index += userSettlementBatchSize) {
      batches.push(uniqueRows.slice(index, index + userSettlementBatchSize));
    }
    return batches;
  }
